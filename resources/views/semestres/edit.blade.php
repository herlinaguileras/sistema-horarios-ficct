                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('semestres.update', $semestre) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $semestre->nombre) }}" required
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_inicio" id="fecha_inicio" 
                                       value="{{ old('fecha_inicio', $semestre->fecha_inicio->format('Y-m-d')) }}" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @error('fecha_inicio')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="date" name="fecha_fin" id="fecha_fin" 
                                       value="{{ old('fecha_fin', $semestre->fecha_fin->format('Y-m-d')) }}" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @error('fecha_fin')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="space-y-3">
                                @foreach ($estados as $estadoOption)
                                    <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition
                                                  {{ old('estado', $semestre->estado) === $estadoOption ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="radio" name="estado" value="{{ $estadoOption }}" 
                                               {{ old('estado', $semestre->estado) === $estadoOption ? 'checked' : '' }}
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm text-blue-700">
                                            Este semestre tiene <strong>{{ $semestre->grupos()->count() }} grupo(s)</strong> asignado(s).
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-yellow-700">
                                        <strong>⚠️ Importante:</strong> Si cambias el estado a "Activo", todos los demás semestres activos se cambiarán automáticamente a "Terminado".
                                    </p>
                                </div>
                            </div>
                        </div>
